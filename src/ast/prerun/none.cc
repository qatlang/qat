    ...
    if (is_type_inferred()) {
      if (!inferredType->is_maybe()) {
        ...
        ctx->Error("The inferred maybe type is " + ctx->color(inferredType->to_string()) +
                 ...
                 inferredType->as_maybe()->is_type_packed()) {
        ctx->Error("The inferred maybe type is " + ctx->color(inferredType->to_string()) + " which is packed, but " +
                       ctx->color("pack") + " is not provided in the none expression. Please change this to " +
