...
ir::Value* MemberFunctionCall::emit(EmitCtx* ctx) {
  ...
  if (isExpSelf) {
    ...
    if (instance->nodeType() == NodeType::SELF) {
      ctx->Error("Do not use this syntax for calling methods. Use " + ctx->color("''" + memberName.value + "(...)") +
                     " instead",
                 fileRange);
    }
  }
  ...
  if (instType->is_expanded()) {
    if (memberName.value == "end") {
      ...
      if (!instType->as_expanded()->has_destructor()) {
        ctx->Error("Type " + ctx->color(instType->as_expanded()->get_full_name()) + " does not have a destructor",
                   fileRange);
      }
    ...
    if (callNature.has_value()) {
        ...
        if (!eTy->has_normal_method(memberName.value)) {
          if (eTy->has_variation(memberName.value)) {
            ctx->Error(ctx->color(memberName.value) + " is a variation method of type " +
                           ctx->color(eTy->get_full_name()) + " and hence cannot be called as a normal method",
                       fileRange);
          } else {
      ...
      if (isVar) {
        if (!eTy->has_valued_method(memberName.value) && !eTy->has_normal_method(memberName.value) &&
          ...
          ctx->Error("Type " + ctx->color(instType->as_expanded()->to_string()) + " does not have a method named " +
                         ctx->color(memberName.value) + ". Please check the logic",
                     fileRange);
        }
      } else {
        if (!eTy->has_valued_method(memberName.value)) {
          if (!eTy->has_normal_method(memberName.value)) {
            if (eTy->has_variation(memberName.value)) {
              ctx->Error(
                  ctx->color(memberName.value) + " is a variation method of type " + ctx->color(eTy->get_full_name()) +
                      " and cannot be called here as this expression " +
                      (inst->is_reference() ? "is a reference without variability" : "does not have variability"),
                  memberName.range);
            } else {
              ctx->Error("Type " + ctx->color(instType->as_expanded()->to_string()) + " does not have a method named " +
                             ctx->color(memberName.value) + ". Please check the logic",
    ...
    auto* memFn = (((callNature.has_value() && callNature.value()) || isVar) && eTy->has_variation(memberName.value))
    ...
    if (!inst->is_ghost_reference() && !inst->is_reference() &&
      ...
      if (instType->is_trivially_copyable() || instType->is_trivially_movable()) {
        ...
        auto origInst = inst;
        inst =
            ir::Value::get(ctx->irCtx->builder.CreateLoad(instType->get_llvm_type(), inst->get_llvm()), instType, true);
      ...
      if (fnArgsTy.at(i)->get_type()->is_reference()) {
        ...
        if (argTy->is_trivially_copyable() || argTy->is_trivially_movable()) {
          ...
          if (!argTy->is_trivially_movable()) {
            ...
            ctx->irCtx->builder.CreateStore(llvm::Constant::getNullValue(argTy->get_llvm_type()), argEmitLLVMValue);
          }
        } else {
          ctx->Error("This expression is not a value and is of type " + ctx->color(argTy->to_string()) +
                         " which is not trivially copyable or trivially movable. Please use " + ctx->color("'copy") +
                         " or " + ctx->color("'move") + " instead",
                     arguments[i - 1]->fileRange);
        }
      }
    ...
    if (memberName.value == "insert") {
      ...
      if (arguments.size() != 1u) {
        ctx->Error("Method " + ctx->color(memberName.value) + " requires " + (arguments.size() > 1 ? "only " : "") +
