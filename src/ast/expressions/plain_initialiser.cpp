...
PlainInitialiser::PlainInitialiser(QatType *_type,
                                   Vec<Pair<String, FileRange>> _fields,
...
IR::Value *PlainInitialiser::emit(IR::Context *ctx) {
  if (type) {
    ...
    if (typeEmit->isCoreType()) {
          ...
          if (dFn->isAccessible(ctx->getReqInfo())) {
            ...
            if (local) {
              if (local->getType()->isMaybe()) {
                alloca = ctx->builder.CreateStructGEP(
                    local->getType()->getLLVMType(), local->getLLVM(), 1u);
              } else {
              ...
              alloca = IR::Logic::newAlloca(ctx->fn, utils::unique_id(),
                                            cTy->getLLVMType());
            }
            (void)dFn->call(ctx, {alloca}, ctx->getMod());
            if (local) {
              auto *val =
                  new IR::Value(local->getLLVM(), local->getType(),
            ...
            ctx->Error("The default constructor of core type " +
      ...
      if (fields.empty()) {
        for (usize i = 0; i < fieldValues.size(); i++) {
          ...
          if (!mem->visibility.isAccessible(reqInfo)) {
            ctx->Error("Member " + ctx->highlightError(mem->name.value) +
                           " of core type " +
                           ctx->highlightError(cTy->getFullName()) +
      ...
      if (local) {
        if (local->getType()->isMaybe()) {
          alloca = ctx->builder.CreateStructGEP(local->getType()->getLLVMType(),
                                                local->getLLVM(), 1u);
        } else {
          alloca = local->getLLVM();
        }
      ...
      if (local) {
        return new IR::Value(local->getLLVM(), local->getType(),
                             local->isVariable(), local->getNature());
      } else {
        return new IR::Value(alloca, cTy, true, IR::Nature::pure);
      }
      ...
      if (fieldValues.size() == 2) {
        ...
        if (dataType->isPointer() &&
            ...
            strData = new IR::Value(
                ctx->builder.CreateLoad(
                    llvm::PointerType::get(llvm::Type::getInt8Ty(ctx->llctx),
                                           0u),
                    ctx->builder.CreateStructGEP(dataType->getLLVMType(),
                                                 strData->getLLVM(), 0u)),
                IR::PointerType::get(
                    false, IR::UnsignedType::get(8u, ctx->llctx),
                    IR::PointerOwner::OfAnonymous(), false, ctx->llctx),
                false, IR::Nature::temporary);
          } else {
            if (strData->isImplicitPointer() ||
                strData->getType()->isReference()) {
          ...
          if (lenType->isUnsignedInteger() &&
            ...
            auto *strAlloca = IR::Logic::newAlloca(ctx->fn, utils::unique_id(),
                                                   strSliceTy->getLLVMType());
            ctx->builder.CreateStore(
                strData->getLLVM(),
                ctx->builder.CreateStructGEP(strSliceTy->getLLVMType(),
                                             strAlloca, 0));
            ctx->builder.CreateStore(
                strLen->getLLVM(),
                ctx->builder.CreateStructGEP(strSliceTy->getLLVMType(),
                                             strAlloca, 1));
            return new IR::Value(strAlloca, strSliceTy, false,
                                 IR::Nature::pure);
          } else {
          ...
          ctx->Error("You are creating an " + ctx->highlightError("str") +
                         ...
                         "should be of " +
                         ctx->highlightError("ptr[u8?]") + " or " +
                         ctx->highlightError("multiptr[u8 ?]") + " type",
          ...
          if (strData->isImplicitPointer() || strData->isReference()) {
            ...
            return new IR::Value(
                ctx->builder.CreatePointerCast(
                    strData->getLLVM(),
                    llvm::PointerType::get(strTy->getLLVMType(), 0u)),
            ...
            return new IR::Value(
                ctx->builder.CreateBitCast(strData->getLLVM(),
                                           strDataTy->getLLVMType()),
                strDataTy, false, IR::Nature::temporary);
          }
        } else {
        ...
        ctx->Error("There are two ways to create a " +
                       ctx->highlightError("str") +
                       ...
                       "ways:\n1) " +
                       ctx->highlightError("ptr[u8 ?]") + " and " +
                       ctx->highlightError("u64") + "\n2) " +
                       ctx->highlightError("multiptr[u8 ?]") + " and " +
                       ctx->highlightError("u64") +
