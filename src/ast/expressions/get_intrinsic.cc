...
ir::Value* GetIntrinsic::emit(EmitCtx* ctx) {
  ...
  if (!ir::StdLib::stdLib->has_choice_type("Intrinsic", AccessInfo::GetPrivileged())) {
    ctx->Error("The choice type " + ctx->color("Intrinsic") +
  ...
  if (nameIR->get_ir_type()->is_same(intrChTy)) {
    ...
    if (nmVal == (u32)IntrinsicID::llvm_matrix_multiply) {
      if (args.size() == 6) {
        ...
        auto mod = ctx->mod;
        auto intrFn =
            llvm::Intrinsic::getDeclaration(mod->get_llvm_module(), llvm::Intrinsic::matrix_multiply,
        ...
        auto fnTy = new ir::FunctionType(ir::ReturnType::get(retTy),
                                         {
                                             ...
                                             new ir::ArgumentType(thirdVal->get_ir_type(), false),
                                             new ir::ArgumentType(thirdVal->get_ir_type(), false),
                                             new ir::ArgumentType(thirdVal->get_ir_type(), false),
                                         },
                                         ctx->irCtx->llctx);
