...
ir::PrerunValue* handle_type_wrap_functions(ir::TypedType* typed, Vec<Expression*> const& arguments,
	...
	if (memberName.value == "byte_size") {
		...
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_integer() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
	} else if (memberName.value == "is_float_type") {
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_float() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
	} else if (memberName.value == "is_tuple_type") {
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_tuple() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
		...
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_struct() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
	} else if (memberName.value == "is_vector_type") {
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_vector() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
		...
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_array() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
	} else if (memberName.value == "is_choice_type") {
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_choice() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
	} else if (memberName.value == "is_maybe_type") {
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_maybe() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
	} else if (memberName.value == "is_result_type") {
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_result() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
		...
		zeroArgCheck();
		return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		                                                   typed->get_subtype()->is_future() ? 1u : 0u),
		                            ir::UnsignedType::create_bool(ctx->irCtx));
		...
		return ir::PrerunValue::get(
		    llvm::ConstantInt::get(
		        llvm::Type::getInt1Ty(ctx->irCtx->llctx),
		        (typed->get_subtype()->is_mark() && typed->get_subtype()->as_mark()->is_slice()) ? 1u : 0u),
		    ir::UnsignedType::create_bool(ctx->irCtx));
