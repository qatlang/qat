...
TypeInfo* TypeInfo::create(ir::Ctx* ctx, Type* type, Mod* mod) {
	...
	if (type->typeInfo) {
		...
		if (not typeInfoType) {
			typeInfoType =
			    llvm::StructType::create({ir::StringSliceType::get(ctx, false)->get_llvm_type()}, "qat.typeid", false);
		}
		if (isPrimitive) {
			if (not builtinModule) {
				builtinModule =
				    Mod::create({"type", FileRange("")}, "", "", ModuleType::file, VisibilityInfo::pub(), ctx);
				builtinGlobal = new llvm::GlobalVariable(
				    *builtinModule->get_llvm_module(), llvm::ArrayType::get(typeInfoType, UINT64_MAX), true,
				    ...
				    builtinModule->get_link_names()
				        ...
				        .toName(),
				    nullptr, llvm::GlobalValue::NotThreadLocal, ctx->dataLayout.value().getDefaultGlobalsAddressSpace(),
				    false);
			}
			builtinTypeInfos.push_back(llvm::ConstantStruct::get(
			    typeInfoType, {StringSliceType::create_value(ctx, type->to_string())->get_llvm_constant()}));
			...
			if (not typeMod->typeInfoDetail) {
				auto infoMod  = Mod::create_submodule(typeMod, "", "", {"type", FileRange("")}, ModuleType::file,
				...
				auto infoList = new llvm::GlobalVariable(
				    *infoMod->get_llvm_module(), llvm::ArrayType::get(typeInfoType, UINT64_MAX), true,
				    ...
				    ctx->dataLayout.value().getDefaultGlobalsAddressSpace(), false);
			...
			typeMod->typeInfoDetail->typeInfos.push_back(llvm::ConstantStruct::get(
			    typeInfoType, {StringSliceType::create_value(
			                       ctx, type->is_type_definition()
	...
	if (not mod->get_llvm_module()->getGlobalVariable(correctGlobalName)) {
		new llvm::GlobalVariable(*mod->get_llvm_module(), llvm::ArrayType::get(typeInfoType, UINT64_MAX), true,
		                         ...
		                         ctx->dataLayout.value().getDefaultGlobalsAddressSpace(), true);
	}
	...
	if (builtinModule) {
		auto newBuiltinGlobal = new llvm::GlobalVariable(
		    *builtinModule->get_llvm_module(), llvm::ArrayType::get(typeInfoType, builtinTypeInfos.size()), true,
		    ...
		    llvm::GlobalVariable::NotThreadLocal, ctx->dataLayout.value().getDefaultGlobalsAddressSpace(), false);
	...
	for (auto* mod : modules) {
		auto newGlobal = new llvm::GlobalVariable(
		    *mod->infoMod->get_llvm_module(), llvm::ArrayType::get(typeInfoType, mod->typeInfos.size()), true,
		    ...
		    llvm::GlobalValue::NotThreadLocal, ctx->dataLayout.value().getDefaultGlobalsAddressSpace(), false);
