...
ir::Skill* SkillEntity::find_skill(EmitCtx* ctx) const {
	...
	auto& ent = names.back();
	if (generics.empty() && mod->has_skill(ent.value, reqInfo)) {
		auto skill = mod->get_skill(ent.value, reqInfo);
		skill->add_mention(ent.range);
		return skill;
	} else if ((not generics.empty()) && mod->has_generic_skill(ent.value, reqInfo)) {
		auto genSkill = mod->get_generic_skill(ent.value, reqInfo);
		...
		ctx->Error(
		    "Could not find a skill named " + ctx->color(ent.value) + " in the module " +
		        ...
		        String(
		            mod->has_generic_skill(ent.value, reqInfo)
		...
		ctx->Error(
		    "Could not find a generic skill named " + ctx->color(ent.value) + " in the module " +
		        ctx->color(mod->get_referrable_name()) +
		        String(
		            mod->has_skill(ent.value, reqInfo)
