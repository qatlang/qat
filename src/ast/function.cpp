...
FunctionPrototype::FunctionPrototype(
    Identifier _name, Vec<Argument *> _arguments, bool _isVariadic,
    ...
    : Node(_fileRange), name(std::move(_name)), isAsync(_is_async),
      ...
      visibility(_visibility), generics(std::move(_generics)),
      linkageType(_linkageType) {}
...
Vec<GenericAbstractType *> FunctionPrototype::getGenerics() const {
  return generics;
}

IR::Function *FunctionPrototype::createFunction(IR::Context *ctx) const {
  auto *mod = ctx->getMod();
  SHOW("Creating function " << name.value
                            << " with generic count: " << generics.size())
  ctx->nameCheckInModule(name, isGeneric() ? "generic function" : "function",
                         isGeneric() ? Maybe<String>(genericFn->getID())
                                     : None);
  ...
  if (isMainFn) {
    if (ctx->getMod()->hasMainFn()) {
      ctx->Error(ctx->highlightError("main") +
                     " function already exists in this module. Please check "
                     "the codebase",
                 fileRange);
    } else {
      if (generatedTypes.size() == 1) {
        if (generatedTypes.at(0)->isPointer() &&
            generatedTypes.at(0)->asPointer()->isMulti() &&
            generatedTypes.at(0)->asPointer()->getOwner().isAnonymous()) {
          if (generatedTypes.at(0)->asPointer()->isSubtypeVariable()) {
            ctx->Error("Type of the first argument of the " +
                           ctx->highlightError("main") +
  ...
  if (isAsync) {
    if (!retTy->isFuture()) {
      ctx->Error("An async function should always return a " +
                     ctx->highlightError("future"),
                 fileRange);
    }
  }
  if (isGeneric()) {
    ...
    auto *fun = IR::Function::Create(mod, Identifier(fnName, name.range),
                                     std::move(genericTypes), retTy, isAsync,
                                     args, isVariadic, fileRange,
    ...
    auto *fun = mod->createFunction(
        Identifier(fnName, name.range), retTy, isAsync, args, isVariadic,
...
void FunctionDefinition::define(IR::Context *ctx) {
  if (!prototype->isGeneric()) {
    ...
    auto *tempFn = new IR::GenericFunction(
        prototype->name, prototype->generics, this, ctx->getMod(),
...
IR::Value *FunctionDefinition::emit(IR::Context *ctx) {
  ...
  if (prototype->isAsync) {
    ...
    auto *entryBlock = llvm::BasicBlock::Create(ctx->llctx, "entry",
                                                fnEmit->getLLVMFunction());
    ...
    SHOW(
        "Linked pthread_create & pthread_attr_init functions to current module")
    auto *pthreadCreate =
        ctx->mod->getLLVMModule()->getFunction("pthread_create");
    auto *pthreadAttrInitFn =
        ctx->mod->getLLVMModule()->getFunction("pthread_attr_init");
    auto *argAlloca = ctx->builder.CreateAlloca(fnEmit->getAsyncArgType());
    auto *pthreadAttrAlloca = ctx->builder.CreateAlloca(
        llvm::StructType::getTypeByName(ctx->llctx, "pthread_attr_t"));
    SHOW("Create allocas for async arg and pthread_attr_t")
    const auto &fnOrigArgs =
        fnEmit->getType()->asFunction()->getArgumentTypes();
    for (usize i = 0; i < fnOrigArgs.size(); i++) {
      auto *argSubGEP = ctx->builder.CreateStructGEP(
          argAlloca->getAllocatedType(), argAlloca, i);
      ctx->builder.CreateStore(fnEmit->getLLVMFunction()->getArg(i), argSubGEP);
    }
    ctx->builder.CreateCall(pthreadAttrInitFn->getFunctionType(),
                            pthreadAttrInitFn, {pthreadAttrAlloca});
    auto *futureTy = fnEmit->getType()
                         ->asFunction()
                         ->getReturnArgType()
                         ->asReference()
                         ->getSubType()
    ...
    auto *futureRefRef = ctx->builder.CreateStructGEP(
        argAlloca->getAllocatedType(), argAlloca, fnOrigArgs.size() - 1);
    auto *pthreadPtr = ctx->builder.CreateStructGEP(
        futureTy->getLLVMType(),
        ctx->builder.CreateLoad(futureTy->getLLVMType()->getPointerTo(),
                                futureRefRef),
        0);
    SHOW("Calling pthread_create")
    ctx->builder.CreateCall(
        pthreadCreate->getFunctionType(), pthreadCreate,
        {pthreadPtr, pthreadAttrAlloca, fun,
         ctx->builder.CreatePointerCast(argAlloca, voidPtrTy)});
    {
      SHOW("Loading future from async arg")
      auto *futureRef = ctx->builder.CreateLoad(
          futureTy->getLLVMType()->getPointerTo(), futureRefRef);
      ...
      ctx->builder.CreateStore(
          ...
          ctx->builder.CreateLoad(
              llvm::Type::getInt1Ty(ctx->llctx)->getPointerTo(),
              ctx->builder.CreateStructGEP(futureTy->getLLVMType(), futureRef,
                                           2u)));
    ...
    auto *asyncArgAlloca = IR::Logic::newAlloca(fnEmit, "qat'asyncArgPtr",
                                                asyncArgTy->getPointerTo());
    ctx->builder.CreateStore(
        ctx->builder.CreatePointerCast(fnEmit->getAsyncSubFunction()->getArg(0),
                                       asyncArgTy->getPointerTo()),
        asyncArgAlloca);
    auto *asyncArg =
        ctx->builder.CreateLoad(asyncArgTy->getPointerTo(), asyncArgAlloca);
    for (usize i = 0; i < fnOrigArgs.size(); i++) {
      IR::LocalValue *localArg = nullptr;
      SHOW("Creating arg at " << i << " for async fn")
      if (i == (fnOrigArgs.size() - 1)) {
        localArg = block->newValue("qat'future", fnOrigArgs.at(i)->getType(),
                                   fnOrigArgs.at(i)->isVariable(),
                                   prototype->name.range);
      } else {
        localArg = block->newValue(fnOrigArgs.at(i)->getName(),
                                   fnOrigArgs.at(i)->getType(),
                                   fnOrigArgs.at(i)->isVariable(),
                                   prototype->arguments.at(i)->getName().range);
      };
      SHOW("Storing arg for async fn")
      SHOW("Arg alloca for future is: "
           << localArg->getType()->asReference()->toString())
      auto *argValLoad = ctx->builder.CreateLoad(
          fnOrigArgs.at(i)->getType()->getLLVMType(),
    ...
    if (isMainFn) {
      ...
      if (fnEmit->getType()->asFunction()->getArgumentCount() == 2u) {
        auto *cmdArgsVal = block->newValue(
            fnEmit->argumentNameAt(0).value.substr(
                0, fnEmit->argumentNameAt(0).value.find('\'')),
            IR::PointerType::get(false, IR::CStringType::get(ctx->llctx),
                                 IR::PointerOwner::OfAnonymous(), true,
                                 ctx->llctx),
            false, fnEmit->argumentNameAt(0).range);
        SHOW("Storing argument pointer")
        ctx->builder.CreateStore(
            fnEmit->getLLVMFunction()->getArg(1u),
            ctx->builder.CreateStructGEP(cmdArgsVal->getType()->getLLVMType(),
                                         cmdArgsVal->getLLVM(), 0u));
        SHOW("Storing argument count")
        ctx->builder.CreateStore(
            ctx->builder.CreateIntCast(fnEmit->getLLVMFunction()->getArg(0u),
                                       llvm::Type::getInt64Ty(ctx->llctx),
                                       true),
            ctx->builder.CreateStructGEP(cmdArgsVal->getType()->getLLVMType(),
                                         cmdArgsVal->getLLVM(), 1u));
      }
    } else {
      ...
      for (usize i = 0; i < argIRTypes.size(); i++) {
        ...
        auto *argVal = block->newValue(
            argIRTypes.at(i)->getName(), argIRTypes.at(i)->getType(),
