...
IR::Value* SayLike::emit(IR::Context* ctx) {
  ...
  if ((sayType == SayType::dbg) ? cfg->isDebugMode() : true) {
    ...
    for (auto* exp : expressions) {
      ...
      SHOW("Say sentence member type: " << valuesIR.back()->getType()->toString());
    }
    std::function<void(IR::Value*, usize)> formatter = [&](IR::Value* value, usize index) {
      SHOW("Say sentence value type is: " << value->getType()->toString())
      auto* val = value;
      auto* typ = val->getType();
      if (typ->isReference()) {
        val->loadImplicitPointer(ctx->builder);
        val = new IR::Value(ctx->builder.CreateLoad(typ->asReference()->getSubType()->getLLVMType(), val->getLLVM()),
      ...
      if (typ->isStringSlice()) {
        ...
        if (typ->asFloat()->getKind() != IR::FloatTypeKind::_64) {
          llvmValues.push_back(ctx->builder.CreateFPCast(val->getLLVM(), llvm::Type::getDoubleTy(ctx->llctx)));
        } else {
          llvmValues.push_back(val->getLLVM());
        }
        ...
        for (usize i = 0; i < subTypes.size(); i++) {
          ...
          subVal       = ctx->builder.CreateGEP(subTypes.at(i)->getLLVMType(), subVal,
                                                llvm::ConstantInt::get(llvm::Type::getInt64Ty(ctx->llctx), i));
          subVal       = ctx->builder.CreateLoad(subTypes.at(i)->getLLVMType(), subVal);
          formatter(new IR::Value(subVal, subTypes.at(i), val->isVariable(), val->getNature()), index);
        ...
        for (usize i = 0; i < typ->asArray()->getLength(); i++) {
          auto* subVal = ctx->builder.CreateGEP(subType->getLLVMType(), val->getLLVM(),
                                                // TODO - Change index type to usize llvm equivalent
                                                llvm::ConstantInt::get(llvm::Type::getInt64Ty(ctx->llctx), i));
          subVal       = ctx->builder.CreateLoad(subType->getLLVMType(), subVal);
          formatter(new IR::Value(subVal, subType, val->isVariable(), val->getNature()), i);
        ...
        ctx->Error("Cannot convert this value to a string", expressions.at(index)->fileRange);
      }
    };
