      ...
      case TypeKind::function: {
        ...
        if (thisVal->get_argument_count() == otherVal->get_argument_count()) {
          if (thisVal->get_return_type()->get_type()->is_same(otherVal->get_return_type()->get_type()) &&
              (thisVal->get_return_type()->is_return_self() == otherVal->get_return_type()->is_return_self())) {
...
TypedType* Type::as_typed() const {
  return (type_kind() == TypeKind::definition) ? ((DefinitionType*)this)->get_subtype()->as_typed() : (TypedType*)this;
}
...
void Type::destroy_value(ir::Ctx* irCtx, ir::Value* instance, ir::Function* fun) {
  if (is_trivially_movable()) {
    ...
    irCtx->Error(
        "Could not destroy an instance of type " + irCtx->color(to_string()) + " as it is not trivially movable", None);
  }
}
...
MixType* Type::as_mix() const {
  return (type_kind() == TypeKind::definition) ? ((DefinitionType*)this)->get_subtype()->as_mix()
...
CType* Type::as_ctype() const {
  return (type_kind() == TypeKind::definition) ? ((DefinitionType*)this)->get_subtype()->as_ctype()
                                               : (is_opaque() ? as_opaque()->get_subtype()->as_ctype() : (CType*)this);
}
