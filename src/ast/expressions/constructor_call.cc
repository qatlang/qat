...
ir::Value* ConstructorCall::emit(EmitCtx* ctx) {
	...
	if (args.empty()) {
		...
		if (not cons->is_accessible(access)) {
			ctx->Error("The default constructor of type " + ctx->color(typ->to_string()) + " is not accessible here",
			           fileRange);
		}
		SHOW("Found default constructor")
	} else if (args.size() == 1) {
		if (not hasFromConvertor(valsType[0])) {
			ctx->Error("No from convertor found for type " + ctx->color(typ->to_string()) + " with type " +
			               ctx->color(valsType.front().second->to_string()),
			           fileRange);
		}
		cons = getFromConvertor(valsType[0]);
		if (not cons->is_accessible(access)) {
			ctx->Error("This convertor of type " + ctx->color(typ->to_string()) + " is not accessible here", fileRange);
		}
		SHOW("Found convertor with type")
	} else {
		if (not hasConstructorWithTypes(valsType)) {
			ctx->Error("No matching constructor found for type " + ctx->color(typ->to_string()), fileRange);
		}
		cons = getConstructorWithTypes(valsType);
		if (not cons->is_accessible(access)) {
			ctx->Error("This constructor of type " + ctx->color(typ->to_string()) + " is not accessible here",
			           fileRange);
		}
	...
	for (usize i = 1; i < argTys.size(); i++) {
		SHOW("Iteration " << i)
		valsIR[i - 1] =
		    ir::Logic::handle_pass_semantics(ctx, argTys.at(i)->get_type(), valsIR[i - 1], args.at(i - 1)->fileRange);
	}
