...
ir::PrerunValue* PrerunMemberFnCall::emit(EmitCtx* ctx) {
  ...
  if (inst->get_ir_type()->is_typed()) {
    return handle_type_wrap_functions(inst->get_ir_type()->as_typed(), arguments, memberName, ctx, fileRange);
  } else {
...
ir::PrerunValue* handle_type_wrap_functions(ir::TypedType* typed, Vec<Expression*> const& arguments,
  ...
  auto zeroArgCheck = [&]() {
    if (arguments.size() != 0) {
      ctx->Error("The type trait " + ctx->color(memberName.value) + " of wrapped type expects zero arguments, but " +
  ...
  if (memberName.value == "byte_size") {
    ...
    return ir::PrerunValue::get(
        llvm::ConstantStruct::get(llvm::cast<llvm::StructType>(ir::StringSliceType::get(ctx->irCtx)->get_llvm_type()),
                                  {ctx->irCtx->builder.CreateGlobalStringPtr(
                                       result, ctx->irCtx->get_global_string_name(), 0U, ctx->mod->get_llvm_module()),
    ...
    zeroArgCheck();
    if (!typed->get_subtype()->get_llvm_type()->isStructTy()) {
    ...
    return ir::PrerunValue::get(
        llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
                               typed->get_subtype()->get_llvm_type()->isStructTy() &&
    ...
    zeroArgCheck();
    return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
                                                       (typed->get_subtype()->is_trivially_copyable()) ||
                                                               (typed->get_subtype()->is_copy_constructible() &&
    ...
    zeroArgCheck();
    return ir::PrerunValue::get(llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->irCtx->llctx),
                                                       (typed->get_subtype()->is_trivially_movable()) ||
                                                               (typed->get_subtype()->is_move_constructible() &&
    ...
    ctx->Error(ctx->color(memberName.value) + " is not a recognised attribute for the wrapped type", memberName.range);
  }
  return nullptr;
}
