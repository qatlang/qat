    ...
    if (theType->is_integer()) {
      ...
      if (eTy->has_default_constructor()) {
        ...
        if (!defFn->is_accessible(ctx->get_access_info())) {
          ctx->Error("The default constructor of type " + ctx->color(eTy->to_string()) + " is not accessible here",
                     fileRange);
        }
      ...
      auto* block = ctx->get_fn()->get_block();
      if (isLocalDecl()) {
        ctx->irCtx->builder.CreateStore(llvm::Constant::getNullValue(mTy->get_llvm_type()), localValue->get_llvm());
        return nullptr;
      } else {
      ...
      if (chTy->has_default()) {
        ...
        ctx->Error("Choice type " + ctx->color(theType->to_string()) + " does not have a default value", fileRange);
      }
      ...
      if (isLocalDecl()) {
        ...
        auto* loc = ctx->get_fn()->get_block()->new_value(irName->value, theType, isVar, irName->range);
        ctx->irCtx->builder.CreateStore(theType->get_prerun_default_value(ctx->irCtx)->get_llvm(), loc->get_llvm());
        return nullptr;
      } else {
