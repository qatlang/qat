...
String LinkNames::toName() const {
  auto isForeign = [&](String const& id) {
    if (foreignID.has_value()) {
      ...
      return parentMod && parentMod->is_in_foreign_mod_of_type(id);
    }
  };
  if (isForeign("C")) {
    ...
    if (units.back().unitType == LinkUnitType::type) {
      return "struct." + cand;
    } else if (units.back().unitType == LinkUnitType::toggle) {
      return "union." + cand;
    } else {
      return cand;
    }
    ...
    for (usize i = 0; i < units.size(); i++) {
      ...
      switch (unit.unitType) {
        ...
        case LinkUnitType::toggle:
          result += "type_toggle_" + unit.name;
          break;
      }
    }
    return result;
