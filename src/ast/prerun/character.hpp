...
class Character final : public PrerunExpression {
  ...
  public:
	Character(bool _isByte, std::array<u8, 4> _character, FileRange _fileRange)
	    : PrerunExpression(std::move(_fileRange)), isByte(_isByte), bytes(_character) {}

	useit static Character* create_char(std::array<u8, 4> character, FileRange fileRange) {
		return std::construct_at(OwnNormal(Character), false, character, std::move(fileRange));
	}

	useit static Character* create_byte(u8 character, FileRange fileRange) {
