		...
		if (typeEmit->is_struct()) {
			...
			if (fields.empty()) {
				for (usize i = 0; i < fieldValues.size(); i++) {
					...
					if (!mem->visibility.is_accessible(reqInfo)) {
						ctx->Error("Member " + ctx->color(mem->name.value) + " of core type " +
				...
				for (usize i = 0; i < fields.size(); i++) {
					...
					if (cTy->has_field_with_name(fName)) {
						...
						if (!mem->visibility.is_accessible(reqInfo)) {
							ctx->Error("The member " + ctx->color(fName) + " of core type " +
				...
				if (fVal->get_ir_type()->is_same(memTy) ||
					...
					ctx->Error("This expression does not match the type of the "
							   "corresponding member " +
								   ctx->color(cTy->get_field_name_at(i)) + " of core type " +
					...
					if (irVal->is_ghost_reference() || irVal->is_reference()) {
						if (memTy->is_trivially_copyable() || memTy->is_trivially_movable()) {
							...
							if (!memTy->is_trivially_copyable()) {
								if (irVal->is_reference() &&
									!irVal->get_ir_type()->as_reference()->isSubtypeVariable()) {
									ctx->Error(
			...
			if (fieldValues.size() == 2) {
				...
				if (dataType->is_mark() && dataType->as_mark()->get_subtype()->is_unsigned_integer() &&
					...
					if (dataType->as_mark()->is_slice()) {
						...
						strData = ir::Value::get(
							strData->is_value()
								...
								: ctx->irCtx->builder.CreateLoad(
									  ...
									  ctx->irCtx->builder.CreateStructGEP(dataType->get_llvm_type(),
																		  strData->get_llvm(), 0u)),
						...
						if (strData->is_ghost_reference() || strData->get_ir_type()->is_reference()) {
							...
							strData = ir::Value::get(
								ctx->irCtx->builder.CreateLoad(dataType->get_llvm_type(), strData->get_llvm()),
								dataType, false);
						}
					}
					if (lenType->is_unsigned_integer() && lenType->as_unsigned_integer()->get_bitwidth() == 64u) {
						if (strLen->is_ghost_reference() || strLen->is_reference()) {
							strLen = ir::Value::get(
								ctx->irCtx->builder.CreateLoad(lenType->get_llvm_type(), strLen->get_llvm()), lenType,
								false);
						}
						...
						ctx->irCtx->builder.CreateStore(
							strLen->get_llvm(),
							ctx->irCtx->builder.CreateStructGEP(strSliceTy->get_llvm_type(), strAlloca, 1));
				...
				if (strDataTy->is_mark() && strDataTy->as_mark()->is_slice() &&
					...
					if (strData->is_ghost_reference() || strData->is_reference()) {
						...
						return ir::Value::get(
							ctx->irCtx->builder.CreateBitCast(strData->get_llvm(), strDataTy->get_llvm_type()),
							strDataTy, false);
					}
				} else {
				...
				ctx->Error(
					"There are two ways to create a " + ctx->color("str") +
			...
			for (auto val : fieldValues) {
				...
				if (!irTy->is_same(vecTy->get_element_type()) &&
					!(irTy->is_reference() &&
					  irTy->as_reference()->get_subtype()->is_same(vecTy->get_element_type()))) {
				...
				if (irTy->is_reference() || irVal->is_ghost_reference()) {
					...
					irVal = ir::Value::get(ctx->irCtx->builder.CreateLoad(irTy->get_llvm_type(), irVal->get_llvm()),
										   irTy, false);
				}
				SHOW("Pushing value")
			...
			if (areAllValuesConstant) {
				if (isLocalDecl()) {
					...
					auto newAlloca =
						ctx->get_fn()->get_block()->new_value(irName.value().value, vecTy, isVar, fileRange);
