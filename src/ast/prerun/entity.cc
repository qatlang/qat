...
ir::PrerunValue* PrerunEntity::emit(EmitCtx* ctx) {
  ...
  if (identifiers.size() == 1 && relative == 0) {
    ...
    if (ctx->has_fn() && ctx->get_fn()->is_method() &&
      ...
      auto* genVal =
          ((ir::Method*)ctx->get_fn())->get_parent_type()->as_expanded()->get_generic_parameter(identifiers[0].value);
      if (genVal->is_typed()) {
        return ir::PrerunValue::get_typed_prerun(ir::TypedType::get(genVal->as_typed()->get_type()));
      } else if (genVal->is_prerun()) {
        return genVal->as_prerun()->get_expression();
  ...
  if (mod->has_type_definition(name.value, reqInfo) || mod->has_brought_type_definition(name.value, reqInfo) ||
    ...
    ctx->Error(ctx->color(name.value) +
               ...
               name.range);
  } else if (mod->has_generic_type_def(name.value, reqInfo) || mod->has_brought_generic_type_def(name.value, reqInfo) ||
    ...
    ctx->Error(ctx->color(name.value) +
               ...
               name.range);
  } else if (mod->has_generic_function(name.value, reqInfo) || mod->has_brought_generic_function(name.value, reqInfo) ||
