#include "./import_bitwidths.hpp"
#include "./types/integer.hpp"
#include "./types/unsigned.hpp"

namespace qat::ast {

void ImportBitwidths::create_module(ir::Mod* mod, ir::Ctx* irCtx) const {
	for (auto typ : importedTypes) {
		if (typ->type_kind() == AstTypeKind::INTEGER) {
			...
			if (mod->has_integer_bitwidth(intAst->bitWidth)) {
				irCtx->Error("This signed integer bitwidth is already imported into the current scope", typ->fileRange);
			} else {
			...
			if (mod->has_unsigned_bitwidth(uintAst->bitWidth)) {
				irCtx->Error("This unsigned integer bitwidth is already imported into the current scope",
				             typ->fileRange);
			} else {
				mod->add_unsigned_bitwidth(uintAst->bitWidth);
			}
		} else {
			irCtx->Error("Expected either a signed or unsigned integer type to be imported into the current scope",
			             typ->fileRange);
		}
		SHOW("Imported integer bitwidth")
	}
}
