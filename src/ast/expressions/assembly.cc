...
ir::Value* AssemblyBlock::emit(EmitCtx* ctx) {
  if (ctx->has_fn()) {
    ...
    if (functionType) {
      ...
      if (!irTy->is_function()) {
        ctx->Error("Expected a function type to represent the nature of the assembly code", functionType->fileRange);
      }
      asmFnTy = irTy->as_function();
    } else {
      asmFnTy = new ir::FunctionType(ir::ReturnType::get(ir::VoidType::get(ctx->irCtx->llctx)), {}, ctx->irCtx->llctx);
    }
    ...
    if (clobberExpr->get_ir_type()->is_array() &&
      ...
      for (usize i = 0; i < itemCount; i++) {
        ...
        clobberString +=
            ir::StringSliceType::value_to_string(ir::PrerunValue::get(item, ir::StringSliceType::get(ctx->irCtx)));
        if (i != (itemCount - 1)) {
          clobberString += ",";
        }
      }
    } else {
      ctx->Error("The list of constraints is expected to be of " + ctx->color("[n]str") + " type", clobbers->fileRange);
    }
    SHOW("Got clobber string")
    if (!clobberString.empty()) {
      auto verifyRes = llvm::InlineAsm::verify(llvm::cast<llvm::FunctionType>(asmFnTy->get_llvm_type()), clobberString);
      if (verifyRes) {
        SHOW("Getting error string")
        auto errStr = llvm::toString(std::move(verifyRes));
        SHOW("Got error string")
        ctx->Error("The resultant constraint string " + ctx->color(clobberString) + " leads to the error: " + errStr,
                   clobbers->fileRange);
      }
    }
    ...
    return ir::Value::get(ctx->irCtx->builder.CreateCall(inlineASM, argsLLVM), asmFnTy->get_return_type()->get_type(),
                          false)
