...
BroughtGroup::BroughtGroup(String _parent, FileRange _fileRange)
    : parent(std::move(_parent)), fileRange(std::move(_fileRange)) {}

BroughtGroup::BroughtGroup(String _parent, Vec<String> _members, FileRange _fileRange)
    : parent(std::move(_parent)), members(std::move(_members)), fileRange(std::move(_fileRange)) {}
...
Json BroughtGroup::toJson() const {
  ...
  return Json()._("parent", parent)._("members", membersJson)._("fileRange", fileRange);
}
