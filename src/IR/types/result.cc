...
ResultType::ResultType(ir::Type* _resTy, ir::Type* _errTy, bool _isPacked, ir::Ctx* irCtx)
	...
	const usize validTypeSize =
	    validType->is_type_sized()
	        ? ((validType->is_opaque() && not validType->as_opaque()->has_subtype())
	               ? validType->as_opaque()->get_deduced_size()
	               : ((usize)(irCtx->dataLayout.getTypeAllocSizeInBits(validType->get_llvm_type()))))
	        : 8u;
	const usize errorTypeSize =
	    errorType->is_type_sized()
	        ? ((errorType->is_opaque() && not errorType->as_opaque()->has_subtype())
	               ? errorType->as_opaque()->get_deduced_size()
	               : ((usize)(irCtx->dataLayout.getTypeAllocSizeInBits(errorType->get_llvm_type()))))
	        : 8u;
...
ir::Type* ResultType::get_valid_type() const { return validType; }
