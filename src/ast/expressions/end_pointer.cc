...
ir::Value* EndPointer::emit(EmitCtx* ctx) {
	...
	if (cand->get_ir_type()->is_ptr()) {
		...
		if (not subTy->is_destructible()) {
			ctx->Error("The value-type of the pointer type " + ctx->color(ptrTy->to_string()) + " is " +
			               ctx->color(subTy->to_string()) +
			               ", which is not destructible. Non-destructible types do not require this operation."
			               " You can use this operation conditionally by checking whether the value-type"
			               " of the pointer type is destructible, using the prerun condition " +
			               ctx->color(String(ptrTy->is_multi() ? "multiPointerExpression" : "pointerExpression") +
			                          "'type'value_type()->is_destructible()"),
			           fileRange);
		}
		if (ptrTy->is_multi()) {
			if (kind == EndPointerKind::PTR) {
				ctx->Error("Invalid syntax for destroying objects in the multi-pointer type " +
				               ctx->color(ptrTy->to_string()) + ". This syntax is meant for pointers only. Use " +
				               ctx->color("'end:multi()") + ", " + ctx->color("'end:from(firstIndexInclusive)") + ", " +
			...
			ir::Block* finalBlock = nullptr;
			if (ptrTy->is_nullable()) {
				const auto currBlock    = ctx->get_fn()->get_block();
				const auto nonNullBlock = ir::Block::create(ctx->get_fn(), currBlock);
				finalBlock              = ir::Block::create(ctx->get_fn(), currBlock->get_parent());
				finalBlock->link_previous_block(currBlock);
				const auto ptrIntTy = llvm::Type::getIntNTy(
				    ctx->irCtx->llctx, ctx->irCtx->dataLayout.getTypeSizeInBits(ptrVal->getType()));
				ctx->irCtx->builder.CreateCondBr(
				    ctx->irCtx->builder.CreateICmpNE(ctx->irCtx->builder.CreatePtrToInt(ptrVal, ptrIntTy),
				                                     llvm::ConstantInt::get(ptrIntTy, 0u)),
				    nonNullBlock->get_bb(), finalBlock->get_bb());
			...
			} else if (kind == EndPointerKind::FROM or kind == EndPointerKind::RANGE) {
				auto fromInd = args[0]->emit(ctx);
				fromInd = ir::Logic::handle_pass_semantics(ctx, fromInd->get_pass_type(), fromInd, args[0]->fileRange);
			...
			if (kind == EndPointerKind::TO || kind == EndPointerKind::RANGE) {
				...
				ir::Logic::panic_in_function(
				    ctx->get_fn(),
				    {
				        ...
				        ir::TextType::create_value(
				            ctx->irCtx, ctx->mod,
				            ", which should be less than or equal to the length of the multi-pointer. The length is "),
				        ir::Value::get(length, ir::NativeType::get_usize(ctx->irCtx), true),
				    },
			...
			ir::Block* restBlock = nullptr;
			if (finalBlock) {
				restBlock = finalBlock;
			} else {
				restBlock = ir::Block::create(ctx->get_fn(), currBlock->get_parent());
				restBlock->link_previous_block(currBlock);
			}
			const auto intPtrTy =
			    llvm::Type::getIntNTy(ctx->irCtx->llctx, ctx->irCtx->dataLayout.getTypeSizeInBits(ptrVal->getType()));
			...
			subTy->destroy_value(
			    ctx->irCtx,
			    ir::Value::get(
			        ctx->irCtx->builder.CreateInBoundsGEP(subTy->get_llvm_type(), ptrVal,
			                                              {ctx->irCtx->builder.CreateLoad(indexTy, index->get_llvm())}),
			        ir::RefType::get(true, ptrTy->get_subtype(), ptrTy->get_address_space(), ctx->irCtx), false),
			    ctx->get_fn());
			...
			if (kind != EndPointerKind::PTR) {
				ctx->Error("Invalid syntax for ending object in the pointer type " + ctx->color(ptrTy->to_string()) +
			...
			if (ptrTy->is_nullable()) {
				...
				restBlock->link_previous_block(currBlock);
				const auto ptrIntTy = llvm::Type::getIntNTy(
				    ctx->irCtx->llctx, ctx->irCtx->dataLayout.getTypeSizeInBits(cand->get_llvm()->getType()));
				ctx->irCtx->builder.CreateCondBr(
				    ctx->irCtx->builder.CreateICmpNE(ctx->irCtx->builder.CreatePtrToInt(cand->get_llvm(), ptrIntTy),
				                                     llvm::ConstantInt::get(ptrIntTy, 0u)),
