...
DestructorDefinition::DestructorDefinition(Vec<Sentence *>  _sentences,
                                           utils::FileRange _fileRange)
    : Node(std::move(_fileRange)), sentences(std::move(_sentences)) {}

void DestructorDefinition::setCoreType(IR::CoreType *_coreType) const {
  coreType = _coreType;
}
...
void DestructorDefinition::define(IR::Context *ctx) {
  ...
  memberFn =
      IR::MemberFunction::CreateDestructor(coreType, fileRange, ctx->llctx);
}
...
IR::Value *DestructorDefinition::emit(IR::Context *ctx) {
  ...
  ctx->builder.CreateStore(memberFn->getLLVMFunction()->getArg(0u),
                           self->getLLVM());
  ctx->selfVal =
      ctx->builder.CreateLoad(corePtrTy->getLLVMType(), self->getAlloca());
  emitSentences(sentences, ctx);
  ctx->selfVal = nullptr;
  IR::functionReturnHandler(ctx, memberFn,
                            sentences.empty() ? fileRange
                                              : sentences.back()->fileRange);
