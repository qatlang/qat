        ...
        if (val->is_value()) {
          ...
          if (typ->is_trivially_copyable() || typ->is_trivially_movable()) {
            ...
            if (!typ->is_trivially_copyable()) {
              ...
              irCtx->builder.CreateStore(llvm::Constant::getNullValue(typ->get_llvm_type()), origVal->get_llvm());
            }
            irCtx->builder.CreateStore(result, gvar);
          } else {
            irCtx->Error("This expression is a reference to the type " + irCtx->color(typ->to_string()) +
                             " which is not trivially copyable or movable. Please use " + irCtx->color("'copy") +
                             " or " + irCtx->color("'move") + " accordingly",
                         fileRange);
          }
        }
      }
