...
ir::Value* LoopInfinite::emit(EmitCtx* ctx) {
		...
		for (const auto& brek : ctx->breakables) {
			if (brek.tag.has_value() && (brek.tag->value == tag->value)) {
				ctx->Error("The tag provided for the loop is already used by another " +
				           ...
				           tag->range, Pair<String, FileRangePtr>{"The existing tag was found here", brek.tag->range});
			}
		}
	...
	SHOW("Infinite loop rest block " << restBlock->get_name())(void)
	    ir::add_branch(ctx->irCtx->builder, trueBlock->get_bb());
