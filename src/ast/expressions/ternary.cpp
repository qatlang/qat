    ...
    if (genType->isInteger() || genType->isUnsignedInteger()) {
      ...
      if (trueVal && falseVal) {
        ...
        if (trueType->isSame(falseType) ||
          ...
          if (trueType->isReference() && (!falseType->isReference() && !falseVal->isImplicitPointer())) {
          ...
          } else if (falseType->isReference() && (!trueType->isReference() && !trueVal->isImplicitPointer())) {
          ...
          falseBlock->setActive(ctx->builder);
          falseBlock->destroyLocals(ctx);
          (void)IR::addBranch(ctx->builder, mergeBlock->getBB());
          mergeBlock->setActive(ctx->builder);
          phiNode = ctx->builder.CreatePHI(resultType->getLLVMType(), 2, utils::unique_id());
