...
MemberFunction::MemberFunction(MemberFnType _fnType, bool _isVariation, CoreType* _parent, const String& _name,
                               QatType* returnType, bool _isReturnTypeVariable, bool _is_async, Vec<Argument> _args,
                               bool is_variable_arguments, bool _is_static, const utils::FileRange& _fileRange,
                               const utils::VisibilityInfo& _visibility_info, llvm::LLVMContext& ctx)
    : Function(_parent->getParent(), _parent->getFullName() + (_is_static ? ":" : "'") + _name, returnType,
...
MemberFunction* MemberFunction::CreateConstructor(CoreType* parent, const Vec<Argument>& args, bool hasVariadicArgs,
  ...
  return new MemberFunction(MemberFnType::constructor, true, parent,
                            "from'" + std::to_string(parent->constructors.size()), VoidType::get(ctx), false, false,
                            argsInfo, hasVariadicArgs, false, fileRange, visibInfo, ctx);
}

MemberFunction* MemberFunction::CreateFromConvertor(CoreType* parent, QatType* sourceType, const String& name,
                                                    const utils::FileRange&      fileRange,
                                                    const utils::VisibilityInfo& visibInfo, llvm::LLVMContext& ctx) {
  Vec<Argument> argsInfo;
  argsInfo.push_back(Argument::Create("''", PointerType::get(true, parent, ctx), 0));
  ...
  return new MemberFunction(MemberFnType::fromConvertor, true, parent, "from'" + sourceType->toString(),
                            VoidType::get(ctx), false, false, argsInfo, false, false, fileRange, visibInfo, ctx);
}

MemberFunction* MemberFunction::CreateToConvertor(CoreType* parent, QatType* destType,
                                                  const utils::FileRange&      fileRange,
                                                  const utils::VisibilityInfo& visibInfo, llvm::LLVMContext& ctx) {
  Vec<Argument> argsInfo;
...
MemberFunction* MemberFunction::CreateStatic(CoreType* parent, const String& name, QatType* returnTy,
...
MemberFunction* MemberFunction::CreateDestructor(CoreType* parent, const utils::FileRange& fileRange,
                                                 llvm::LLVMContext& ctx) {
...
MemberFunction* MemberFunction::CreateOperator(CoreType* parent, bool isBinary, bool isVariationFn, const String& opr,
  ...
  return new MemberFunction(
      ...
      "operator'" + opr + (isBinary ? ("'" + std::to_string(parent->getOperatorVariantIndex(opr))) : ""), returnType,
      false, false, args_info, false, false, fileRange, visibInfo, ctx);
}
