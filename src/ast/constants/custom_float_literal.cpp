...
IR::PrerunValue* CustomFloatLiteral::emit(IR::Context* ctx) {
  ...
  if (kind == "f16") {
    return new IR::PrerunValue(llvm::ConstantFP::get(llvm::Type::getHalfTy(ctx->llctx), std::stof(value)),
                               IR::FloatType::get(IR::FloatTypeKind::_16, ctx->llctx));
  } else if (kind == "fbrain") {
    return new IR::PrerunValue(llvm::ConstantFP::get(llvm::Type::getBFloatTy(ctx->llctx), std::stof(value)),
                               IR::FloatType::get(IR::FloatTypeKind::_brain, ctx->llctx));
  } else if (kind == "f32") {
    return new IR::PrerunValue(llvm::ConstantFP::get(llvm::Type::getFloatTy(ctx->llctx), std::stof(value)),
                               IR::FloatType::get(IR::FloatTypeKind::_32, ctx->llctx));
  } else if (kind == "f64") {
    return new IR::PrerunValue(llvm::ConstantFP::get(llvm::Type::getDoubleTy(ctx->llctx), std::stod(value)),
                               IR::FloatType::get(IR::FloatTypeKind::_64, ctx->llctx));
  } else if (kind == "f80") {
    return new IR::PrerunValue(llvm::ConstantFP::get(llvm::Type::getX86_FP80Ty(ctx->llctx), std::stod(value)),
                               IR::FloatType::get(IR::FloatTypeKind::_80, ctx->llctx));
  } else if (kind == "f128") {
    return new IR::PrerunValue(llvm::ConstantFP::get(llvm::Type::getFP128Ty(ctx->llctx), std::stod(value)),
                               IR::FloatType::get(IR::FloatTypeKind::_128, ctx->llctx));
  } else if (kind == "f128ppc") {
    return new IR::PrerunValue(llvm::ConstantFP::get(llvm::Type::getPPC_FP128Ty(ctx->llctx), std::stod(value)),
