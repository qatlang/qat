...
String Logic::getTemplateVariantName(String              mainName,
...
llvm::AllocaInst *Logic::newAlloca(IR::Function *fun, const String &name,
                                   llvm::Type *type) {
  llvm::AllocaInst *result = nullptr;
  if (fun->getLLVMFunction()->getEntryBlock().getInstList().empty()) {
    result = new llvm::AllocaInst(type, 0U, name,
...
IR::Value *Logic::handleCopyOrMove(IR::Value *value, IR::Context *ctx,
  ...
  if (value->getType()->isCoreType()) {
    if (value->isImplicitPointer() || value->isReference()) {
      ...
      if (cTy->hasCopyConstructor()) {
        ...
        return new IR::Value(copy,
                             IR::ReferenceType::get(true, cTy, ctx->llctx),
                             false, IR::Nature::temporary);
      } else if (cTy->hasMoveConstructor()) {
        auto *mvFn = cTy->getMoveConstructor();
        auto *moved =
            newAlloca(ctx->fn, utils::unique_id(), cTy->getLLVMType());
        (void)mvFn->call(ctx, {moved, value->getLLVM()}, ctx->getMod());
        ctx->Warning(
            "A move is happening here. Considering making it explicit so that "
            "there is no confusion as to the validity of the moved value",
            fileRange);
        return new IR::Value(moved,
                             IR::ReferenceType::get(true, cTy, ctx->llctx),
                             false, IR::Nature::temporary);
