      ...
      if (subRes.first) {
        if (subRes.second) {
          if (!expression.has_value()) {
            ctx->Error("Field " + ctx->color(subName.value) + " of mix type " + ctx->color(mixTy->get_full_name()) +
        ...
        if (isLocalDecl()) {
          ...
          createIn =
              ctx->get_fn()->get_block()->new_value(irName.has_value() ? irName->value : utils::unique_id(), mixTy,
