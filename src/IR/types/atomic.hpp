...
class AtomicType final : public Type {
	...
	llvm::AtomicOrdering get_llvm_ordering(ir::Ctx* irCtx) const;
  ...
  public:
	...
	Maybe<AtomicOrdering> get_ordering() const { return ordering; }

	static String ordering_to_string(AtomicOrdering order) {
		switch (order) {
	...
	bool can_be_prerun_generic() const final { return false; }

	bool has_prerun_default_value() const final { return true; }

	bool is_type_sized() const final { return true; }

	bool is_default_constructible() const final { return subType->has_prerun_default_value(); }

	bool is_copy_constructible() const final { return true; }

	bool is_copy_assignable() const final { return true; }

	bool is_move_constructible() const final { return true; }

	bool is_move_assignable() const final { return true; }

	bool is_destructible() const final { return true; }

	bool has_simple_copy() const final { return false; }

	bool has_simple_move() const final { return false; }
	...
	TypeKind type_kind() const final { return TypeKind::ATOMIC; }
};
