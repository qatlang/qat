#include "./vector.hpp"
#include "../../utils/qat_region.hpp"

#include <llvm/IR/DerivedTypes.h>

namespace qat::ir {

Vec<VectorType*> VectorType::allVectorTypes = {};

VectorType::VectorType(ir::Type* _subType, usize _count, VectorKind _kind, ir::Ctx*)
    : subType(_subType), count(_count), kind(_kind) {
	llvmType = kind == VectorKind::fixed ? (llvm::Type*)llvm::FixedVectorType::get(subType->get_llvm_type(), count)
	                                     : llvm::ScalableVectorType::get(subType->get_llvm_type(), count);
	allVectorTypes.push_back(this);
}

VectorType* VectorType::create(ir::Type* subType, usize count, VectorKind kind, ir::Ctx* irCtx) {
	for (auto typ : allVectorTypes) {
		if (typ->subType->is_same(subType) && (typ->count == count) && (typ->kind == kind)) {
			return typ;
		}
	}
	return std::construct_at(OwnNormal(VectorType), subType, count, kind, irCtx);
}

String VectorType::to_string() const {
	return String("vec:[") + ((kind == VectorKind::scalable) ? "?, " : "") + std::to_string(count) + ", " +
	       subType->to_string() + "]";
}

} // namespace qat::ir
