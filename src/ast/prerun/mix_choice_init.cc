...
ir::PrerunValue* PrerunMixOrChoiceInit::emit(EmitCtx* ctx) {
  ...
  if (typeEmit->is_mix()) {
    ...
    if (!mixTy->can_be_prerun()) {
      ctx->Error("Mix type " + ctx->color(mixTy->to_string()) + " cannot be used in a prerun expression", fileRange);
    }
    ...
    if (subRes.first) {
      if (subRes.second) {
        if (!expression.has_value()) {
          ctx->Error("Variant " + ctx->color(subName.value) + " of mix type " + ctx->color(mixTy->get_full_name()) +
    ...
    ctx->Error(ctx->color(typeEmit->to_string()) + " is not a mix type or a choice type and hence cannot be used here",
               fileRange);
  }
  return nullptr;
}
