...
ir::Value* MetaIntrinsic::emit(EmitCtx* ctx) {
	...
	if (nameIR->get_ir_type()->is_same(intrChTy)) {
		...
		if (nmVal == (u32)IntrinsicID::matrix_multiply) {
			if (args.size() != 6) {
				ctx->Error(
				    "This intrinsic requires 5 parameters to be provided after the Intrinsic ID. The first two parameters are the"
				    " underlying vectors types of the matrices, the 3rd parameter is the number of rows of the first matrix,"
				    " the 4th parameter is the number of columns of the first matrix which is equal to the number of rows of the"
				    " second matrix, and the 5th parameter is the number of columns of the second matrix",
				    fileRange);
			}
			...
			if (not llvm::cast<llvm::ConstantInt>(
			            ...
			            .getBoolValue()) {
				ctx->Error(
				    "The first type provided is " + ctx->color(oneTy->to_string()) +
			...
			if (not llvm::cast<llvm::ConstantInt>(
			            ...
			            .getBoolValue()) {
				ctx->Error(
				    "The second type provided is " + ctx->color(oneTy->to_string()) +
			...
			if (args.size() != 3) {
				ctx->Error(
				    "This intrinsic requires 3 parameters to be provided after the Intrinsic ID. The first parameter"
				    " is the vector type of the matrix. The second parameter is the number of rows of the matrix and"
				    " the third parameter is the number of columns of the matrix",
				    fileRange);
			}
			auto firstVal = args[0]->emit(ctx);
			if (not firstVal->get_ir_type()->is_typed()) {
			...
			auto secondVal = args[1]->emit(ctx);
			if (not(secondVal->get_ir_type()->is_unsigned() &&
			        (secondVal->get_ir_type()->as_unsigned()->get_bitwidth() == 32u))) {
				ctx->Error("This value is expected to be of type " + ctx->color("u32") +
				               ". Got an expression of type " + ctx->color(secondVal->get_ir_type()->to_string()),
				           args[1]->fileRange);
			}
			auto thirdVal = args[2]->emit(ctx);
			if (not(thirdVal->get_ir_type()->is_unsigned() &&
			        (thirdVal->get_ir_type()->as_unsigned()->get_bitwidth() == 32u))) {
				ctx->Error("This value is expected to be of type " + ctx->color("u32") +
				               ". Got an expression of type " + ctx->color(thirdVal->get_ir_type()->to_string()),
				           args[2]->fileRange);
			}
			...
			if (not llvm::cast<llvm::ConstantInt>(llvm::ConstantFoldCompareInstruction(
			            ...
			            .getBoolValue()) {
				ctx->Error(
				    "The first type provided is " + ctx->color(firstTy->to_string()) +
				        " but the product of the 2nd and 3rd values is " +
				        ctx->color(std::to_string(*llvm::cast<llvm::ConstantInt>(
				                                       llvm::ConstantFoldConstant(oneMulRes, ctx->irCtx->dataLayout))
				                                       ->getValue()
				                                       .getRawData())) +
			...
			auto fnTy =
			    ir::FunctionType::create(ir::ReturnType::get(firstTy),
			                             {ir::ArgumentType::create_normal(firstTy, None, false),
			                              ir::ArgumentType::create_normal(secondVal->get_ir_type(), None, false),
			                              ir::ArgumentType::create_normal(thirdVal->get_ir_type(), None, false)},
			                             ctx->irCtx->llctx);
			return ir::Value::get(intrFn, fnTy, false);
		} else {
...
Json MetaIntrinsic::to_json() const {
	...
	return Json()._("nodeType", "metaIntrinsic")._("parameters", vals)._("fileRange", fileRange);
}
