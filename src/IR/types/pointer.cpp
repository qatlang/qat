...
PointerOwner PointerOwner::OfHeap() {
  return PointerOwner{.owner = nullptr, .ownerTy = PointerOwnerType::heap};
}
...
PointerOwner PointerOwner::OfFunction(Function *fun) {
  return PointerOwner{.owner = (void *)fun,
                      .ownerTy = PointerOwnerType::function};
}
...
Region *PointerOwner::ownerAsRegion() const {
  return ((QatType *)owner)->asRegion();
}
...
bool PointerOwner::isAnonymous() const {
  return ownerTy == PointerOwnerType::anonymous;
}

bool PointerOwner::isRegion() const {
  return ownerTy == PointerOwnerType::region;
}

bool PointerOwner::isHeap() const { return ownerTy == PointerOwnerType::heap; }

bool PointerOwner::isFunction() const {
  return ownerTy == PointerOwnerType::function;
}
...
bool PointerOwner::isSame(const PointerOwner &other) const {
  if (ownerTy == other.ownerTy) {
    ...
    case PointerOwnerType::region: // FIXME - Change this once region is
                                   // implemented!!!
      return true;
...
PointerType::PointerType(bool _isSubtypeVariable, QatType *_type,
                         PointerOwner _owner, bool _hasMulti,
  ...
  if (_hasMulti) {
    if (llvm::StructType::getTypeByName(ctx, "multiptr[" + subType->toString() +
                                                 "]")) {
      llvmType = llvm::StructType::getTypeByName(
          ctx, "multiptr[" + subType->toString() + "]");
    } else {
      llvmType = llvm::StructType::create(
          {llvm::PointerType::get(subType->getLLVMType()->isVoidTy()
  ...
  for (auto *typ : types) {
    if (typ->isPointer()) {
      if (typ->asPointer()->getSubType()->isSame(_type) &&
          ...
          (typ->asPointer()->isMulti() == _hasMulti)) {
...
String PointerType::toString() const {
  return String(isMulti() ? "multiptr[" : "ptr[") +
         String(isSubtypeVariable() ? "var " : "") + subType->toString() + " " +
         owner.toString() + "]";
}
