...
ir::Value* LoopIn::emit(EmitCtx* ctx) {
    ...
    if (brk.tag.has_value()) {
      ...
      if (indexName.has_value() && (brk.tag->value == indexName->value)) {
        ctx->Error("The name for the loop index provided here is already used by another " +
                       ctx->color(brk.type_to_string()),
                   indexName->range, Pair<String, FileRange>{"The existing tag was found here", brk.tag->range});
      }
    }
  }
  ...
  if (isTyArray || isTySlice || isTyCString || isTyStrSlice || isTyVec) {
    ...
    ir::Type*    countTy = ir::CType::get_usize(ctx->irCtx);
    ...
    if (isTyArray) {
      ...
      if (isRefUnder) {
        candExp = ir::Value::get(ctx->irCtx->builder.CreateLoad(elemTy->get_llvm_type(), candExp->get_llvm()), candType,
                                 true);
      }
      if (candType->as_vector()->is_fixed()) {
        lenVal = llvm::ConstantInt::get(llvm::Type::getInt64Ty(ctx->irCtx->llctx), candType->as_vector()->get_count());
      } else {
        lenVal = ctx->irCtx->builder.CreateMul(
            ctx->mod->link_intrinsic(ir::IntrinsicID::vectorScale),
            llvm::ConstantInt::get(llvm::Type::getInt64Ty(ctx->irCtx->llctx), candType->as_vector()->get_count()));
      }
    }
    ...
    auto indexVar  = mainBlock->new_value(indexName.has_value() ? indexName->value : utils::unique_id(), countTy, false,
                                         indexName.has_value() ? indexName->range : fileRange);
    ...
    ctx->irCtx->builder.CreateStore(llvm::ConstantInt::get(countTy->get_llvm_type(), 0u, false), indexVar->get_llvm());
        ...
        isTyCString
            ? ctx->irCtx->builder.CreateAnd(
                  ctx->irCtx->builder.CreateICmpNE(
                      ...
                      llvm::ConstantInt::get(ir::CType::get_ptrdiff_unsigned(ctx->irCtx)->get_llvm_type(), 0, false)),
                  ctx->irCtx->builder.CreateICmpNE(
                      ctx->irCtx->builder.CreateLoad(llvm::IntegerType::getInt8Ty(ctx->irCtx->llctx),
                                                     candExp->get_llvm()),
                      zeroU8))
            : ctx->irCtx->builder.CreateICmpULT(llvm::ConstantInt::get(countTy->get_llvm_type(), 0u, false), lenVal),
    ...
    if (isTyArray) {
      llItemVal = ctx->irCtx->builder.CreateInBoundsGEP(
          ...
          {zero64, ctx->irCtx->builder.CreateLoad(indexVar->get_ir_type()->get_llvm_type(), indexVar->get_llvm())});
    } else if (isTyVec) {
    ...
    if (isTyCString) {
      ...
      loopCond = ctx->irCtx->builder.CreateICmpULT(
          ctx->irCtx->builder.CreateAdd(ctx->irCtx->builder.CreateLoad(countTy->get_llvm_type(), indexVar->get_llvm()),
                                        llvm::ConstantInt::get(zero64->getType(), 1u, false)),
          lenVal);
    }
