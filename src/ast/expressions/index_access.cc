...
ir::Value* IndexAccess::emit(EmitCtx* ctx) {
  ...
  if (instType->is_tuple()) {
      ...
      if (instType->is_mark()) {
        ...
        if (llvm::isa<llvm::ConstantInt>(ind->get_llvm())) {
          ...
          if (llvm::cast<llvm::ConstantInt>(ind->get_llvm())->isZero()) {
            ...
            return ir::Value::get(
                ctx->irCtx->builder.CreateInBoundsGEP(instType->get_llvm_type(), inst->get_llvm(), {zero64, zero64}),
      ...
      if (ind->is_reference()) {
        ind = ir::Value::get(
            ctx->irCtx->builder.CreateLoad(ind->is_reference()
                                               ? ind->get_ir_type()->as_reference()->get_subtype()->get_llvm_type()
                                               : ind->get_ir_type()->get_llvm_type(),
                                           ind->get_llvm()),
            ind->is_reference() ? ind->get_ir_type()->as_reference()->get_subtype() : ind->get_ir_type(), false);
      ...
      if (llvm::cast<llvm::ConstantInt>(
        ...
        return ir::PrerunValue::get(
            llvm::ConstantInt::get(
                llvm::Type::getInt8Ty(ctx->irCtx->llctx),
                llvm::cast<llvm::ConstantDataArray>(inst->get_llvm_constant()->getAggregateElement(0u)->getOperand(0u))
                    ->getRawDataValues()
                    .str()
    ...
    return ir::Value::get(
        ...
        ir::ReferenceType::get(isVarExp, ir::CType::get_char(ctx->irCtx), ctx->irCtx), false);
    ...
    if (!indType->is_reference() &&
        ...
        if (ind->is_ghost_reference()) {
          if (indType->is_trivially_copyable() || indType->is_trivially_movable()) {
            ...
            if (!indType->is_trivially_copyable()) {
              ...
              ctx->irCtx->builder.CreateStore(llvm::Constant::getNullValue(indType->get_llvm_type()), ind->get_llvm());
            }
            ind = indVal;
          } else {
      ...
      if (indSubType->is_trivially_copyable() || indSubType->is_trivially_movable()) {
        ...
        if (!indType->is_trivially_copyable()) {
          ...
          ctx->irCtx->builder.CreateStore(llvm::Constant::getNullValue(indSubType->get_llvm_type()), ind->get_llvm());
        }
        ind = indVal;
      } else {
        ctx->Error("This expression is a reference to type " + ctx->color(indSubType->to_string()) +
                       " which cannot be trivially copied or trivially moved. Please use " + ctx->color("'copy") +
