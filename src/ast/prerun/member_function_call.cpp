...
IR::PrerunValue* PrerunMemberFnCall::emit(IR::Context* ctx) {
  ...
  if (inst->getType()->isTyped()) {
    return handle_type_wrap_functions(inst->getType()->asTyped(), arguments, memberName, ctx, fileRange);
  } else {
...
IR::PrerunValue* handle_type_wrap_functions(IR::TypedType* typed, Vec<PrerunExpression*> const& arguments,
...
IR::PrerunValue* handle_type_wrap_functions(IR::TypedType* typed, Vec<Expression*> const& arguments,
                                            Identifier memberName, IR::Context* ctx, FileRange fileRange) {
  auto zeroArgCheck = [&]() {
    if (arguments.size() != 0) {
  ...
  if (memberName.value == "byte_size") {
    ...
    return new IR::PrerunValue(
        llvm::ConstantInt::get(llvm::Type::getInt64Ty(ctx->llctx),
                               subTy->isTypeSized() ? ctx->getMod()->getLLVMModule()->getDataLayout().getTypeStoreSize(
                                                          subTy->getLLVMType())
                                                    : 1u),
    ...
    return new IR::PrerunValue(
        llvm::ConstantStruct::get(
            ...
            {ctx->builder.CreateGlobalStringPtr(result, ctx->getGlobalStringName(), 0U, ctx->getMod()->getLLVMModule()),
    ...
    if (!typed->getSubType()->getLLVMType()->isStructTy()) {
      ctx->Error("The wrapped type is not a struct and hence the type trait " + ctx->highlightError(memberName.value) +
                     " cannot be used. Make sure to check the " + ctx->highlightError("is_underlying_struct_type") +
    ...
    return new IR::PrerunValue(
        llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->llctx),
                               (typed->getSubType()->isPointer() && typed->getSubType()->asPointer()->isMulti()) ? 1u
                                                                                                                 : 0u),
        IR::UnsignedType::getBool(ctx));
  } else if (memberName.value == "is_trivially_copyable") {
    zeroArgCheck();
    return new IR::PrerunValue(
        llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->llctx), typed->getSubType()->isTriviallyCopyable() ? 1u : 0u),
        IR::UnsignedType::getBool(ctx));
  } else if (memberName.value == "is_trivially_movable") {
    zeroArgCheck();
    return new IR::PrerunValue(
        llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->llctx), typed->getSubType()->isTriviallyMovable() ? 1u : 0u),
        IR::UnsignedType::getBool(ctx));
  } else if (memberName.value == "is_copy_constructible") {
    zeroArgCheck();
    return new IR::PrerunValue(
        llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->llctx), typed->getSubType()->isCopyConstructible() ? 1u : 0u),
        IR::UnsignedType::getBool(ctx));
    ...
    zeroArgCheck();
    return new IR::PrerunValue(
        llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->llctx), typed->getSubType()->isMoveConstructible() ? 1u : 0u),
        IR::UnsignedType::getBool(ctx));
    ...
    zeroArgCheck();
    return new IR::PrerunValue(
        llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->llctx), (typed->getSubType()->isTriviallyCopyable()) ||
    ...
    zeroArgCheck();
    return new IR::PrerunValue(
        llvm::ConstantInt::get(llvm::Type::getInt1Ty(ctx->llctx), (typed->getSubType()->isTriviallyMovable()) ||
