  ...
  if (mod->has_generic_function(entityName.value, reqInfo) ||
    ...
    for (usize i = 0; i < genericTypes.size(); i++) {
      ...
      if (gen->is_prerun() && (gen->as_prerun()->nodeType() == NodeType::PRERUN_DEFAULT)) {
        ...
        ((ast::PrerunDefault*)(genericTypes.at(i)->as_prerun()))->setGenericAbstract(genericFn->getGenericAt(i));
    ...
    if (handleRes.has_value()) {
      ...
      auto fullName = Identifier::fullName(names);
      ctx->Error("No generic function or type named " + ctx->color(fullName.value) + " found in the current scope",
                 fullName.range);
    }
  }
  return nullptr;
