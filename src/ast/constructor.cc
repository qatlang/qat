...
void ConstructorPrototype::define(MethodState& state, ir::Ctx* irCtx) {
  ...
  if (type == ConstructorType::normal) {
    ...
    for (auto* arg : arguments) {
      if (arg->is_type_member()) {
        ...
        if (state.parent->get_parent_type()->is_struct()) {
          ...
          if (!check.first) {
            irCtx->Error("No variant named " + irCtx->color(arg->get_name().value) + " is present in mix type " +
                             irCtx->color(mixTy->to_string()),
                         arg->get_name().range);
          }
          if (!check.second) {
            irCtx->Error("The variant " + irCtx->color(arg->get_name().value) + " of mix type " +
                             irCtx->color(mixTy->to_string()) +
    ...
    if (state.parent->is_done_skill() && state.parent->as_done_skill()->has_constructor_with_types(generatedTypes)) {
      irCtx->Error("A constructor with the same signature exists in the same implementation " +
  ...
  if (state.parent->get_parent_type()->is_struct()) {
    ...
    for (usize i = 0; i < coreTy->get_field_count(); i++) {
      ...
      if (mem->defaultValue.has_value()) {
        ...
        } else if (memVal->is_reference() && memVal->get_ir_type()->as_reference()->get_subtype()->is_same(mem->type)) {
          if (mem->type->is_trivially_copyable() || mem->type->is_trivially_movable()) {
            irCtx->builder.CreateStore(irCtx->builder.CreateLoad(mem->type->get_llvm_type(), memVal->get_llvm()),
                                       irCtx->builder.CreateStructGEP(coreTy->get_llvm_type(), self->get_llvm(), i));
            if (!mem->type->is_trivially_copyable()) {
              ...
              irCtx->builder.CreateStore(llvm::Constant::getNullValue(mem->type->get_llvm_type()), memVal->get_llvm());
            }
          } else {
        ...
        if (mem->type->has_prerun_default_value()) {
          irCtx->Warning("Member field " + irCtx->highlightWarning(mem->name.value) +
                             ...
                             " at the end of this constructor. Try using " + irCtx->highlightWarning("default") +
          ...
          irCtx->Warning("Member field " + irCtx->highlightWarning(mem->name.value) +
                             ...
                             irCtx->highlightWarning("default") + " instead to explicitly initialise the member field",
                         fileRange);
